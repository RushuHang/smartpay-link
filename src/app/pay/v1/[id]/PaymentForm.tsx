...
import { CreditCard, User, Mail, Phone, Calendar, ShieldCheck, Wallet, AlertCircle, ChevronRight } from "lucide-react";
...
export default function PaymentForm() {
    ...
    return (
      <div className="w-full min-h-screen flex items-center justify-center p-4 font-sans">
        ...
        >
          ...
          <p className="text-slate-500 text-sm mt-2 mb-8">This payment link is either expired or does not exist.</p>
          <Button
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-md bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-100 p-8 sm:p-10 font-sans"
    >
      {/* Header Section */}
      <div className="text-center mb-8">
        <p className="text-lg font-bold text-brand-primary">Smart Link</p>

        <div className="mt-2 flex items-center justify-center gap-2 text-slate-500">
          <span className="text-sm">Invoice #INV-2026-042</span>
          {data.allowPartialPayment && (
            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700">Partial Allowed</span>
          )}
        </div>
      </div>
      ...
      <div className="space-y-5 mb-8 ">
        ...
        <div className="flex items-center justify-between group">
          ...
          <span className="text-sm font-semibold text-slate-900">{data.description}</span>
        </div>
        <div className="flex items-center justify-between group">
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium text-slate-500">Invoice Date</span>
          </div>
      ...
      <div className="pt-2 space-y-4 text-center">
        <motion.div whileTap={{ scale: 0.98 }}>
          <Button className="w-ful px-16 py-4 group">
            Pay Now <ChevronRight className="group-hover:translate-x-1 transition-transform ease-in-out" size={18} />
          </Button>
        </motion.div>
        ...
        <p className="text-sm font-medium text-slate-500">
          Having trouble?{" "}
          <Link className="hover:underline text-slate-700" href="#">
            Contact Support
          </Link>
        </p>
      </div>
